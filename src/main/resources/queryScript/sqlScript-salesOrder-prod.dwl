...
ON 
    ...
    AND f4.IVXRT = 'C' 
    AND TRIM(f4.IVEFTJ) <= $(vars.previousSalesOrderJobRun.date) 
    AND TRIM(f4.IVEXDJ) >= $(vars.previousSalesOrderJobRun.date)
LEFT JOIN 
    PRODCTL.F0005 f5 
ON 
    TRIM(f5.DRKY) = 
...
WHERE
    TRIM(f1.SDUPMJ) >= $(vars.previousSalesOrderJobRun.date)
    AND TRIM(f1.SDTDAY) >= $(vars.previousSalesOrderJobRun.time) 
